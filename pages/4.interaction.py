...
for idx in range(len(st.session_state.all_messages)):
    ...
    if int(idx+1) in st.session_state.dict_imgs:
        st.image(st.session_state.dict_imgs[int(idx+1)], use_column_width=True)
...
if 'interaction_messages' not in st.session_state:
    ...
    st.session_state.interaction_messages.append({"role" : "system", "content" : 
        f'''
        너는 아이의 한국어와 제 2언어인 {st.session_state.select_lang_name}의 언어 이해 능력을 키우는 GPT야. 
        ...
        2. 부모의 답변: 네 괜찮습니다
        3. GPT의 질문: 퍼플이 블럭으로 풋볼 경기장을 만들었어! 너는 블럭이나 장난감으로 뭘 만들어 본 적이 있어? 😊
        4. 아이의 답변: 블록으로 경찰차를 만들었어요
        5. GPT의 호응: 와! 경찰차를 만들었다니 멋지다! 🚔 이번에는 내가 한 말을 영어로 엄마에게 말해볼까? 엄마랑 같이 말해봐도 좋아! 😊
        ...
        4. 아이의 답변: 함께 맛있는 음식을 먹어
        5. GPT의 호응: 정말 좋겠다! 가족과 함께 먹는 음식은 더 맛있지. 😊 이제 내가 한 질문을 중국어로 엄마에게 물어봐!

        다음의 요소들과 생성된 동화 이야기를 참고해서 위 예시들과 유사하게 진행해줘.
        - 부모의 선호 요소는 {st.session_state.parent_prefer} 이고 아이의 선호 요소는 {st.session_state.child_prefer}
        - 생성된 동화 이야기: {generated_korean_fairytale}
        '''
    })
